...
public class Board {
    ...
    public void generateKing() {
        this.mCases[0][4] = new Case(0, 4, new King(0, 4, -1));
        this.mCases[7][3] = new Case(7, 3, new King(7, 3, 1));
    }

    public void generateQueen() {
        this.mCases[0][3] = new Case(0, 3, new Queen(0, 3, -1));
        this.mCases[7][4] = new Case(7, 4, new Queen(7, 4, 1));
    }

    public void generateBishop() {
        this.mCases[0][2] = new Case(0, 2, new Bishop(0, 2, -1));
        this.mCases[0][5] = new Case(0, 5, new Bishop(0, 5, -1));
        this.mCases[7][2] = new Case(7, 2, new Bishop(7, 2, 1));
        this.mCases[7][5] = new Case(7, 5, new Bishop(7, 5, 1));
    }

    public void generateKnight() {
        this.mCases[0][1] = new Case(0, 1, new Knight(0, 1, -1));
        this.mCases[0][6] = new Case(0, 6, new Knight(0, 6, -1));
        this.mCases[7][1] = new Case(7, 1, new Knight(7, 1, 1));
        this.mCases[7][6] = new Case(7, 6, new Knight(7, 6, 1));
    }

    public void generateRook() {
        this.mCases[0][0] = new Case(0, 0, new Rook(0, 0, -1));
        this.mCases[0][7] = new Case(0, 7, new Rook(0, 7, -1));
        this.mCases[7][0] = new Case(7, 0, new Rook(7, 0, 1));
        this.mCases[7][7] = new Case(7, 7, new Rook(7, 7, 1));
    }

    public void generatePawns() {
        for (int j = 0; j < 8; j++) {
            this.mCases[1][j] = new Case(1, j, new Pawn(1, j, -1));
            this.mCases[6][j] = new Case(6, j, new Pawn(6, j, 1));
        }
    }
    ...
    public boolean checkMove(int ox, int oy, int dx, int dy, Piece piece) {
        ...
        if (piece.getType() != "Knight") {
            while (x != dx || y != dy) {
                ...
                System.out.println("dir1: " + piece.getDir());
                System.out.println("dir1: " + piece.getMoves()[piece.getDir()][1] + " dir2: "
                        + piece.getMoves()[piece.getDir()][0]);
                System.out.println("x: " + x + " y: " + y);
                System.out.println("dx: " + dx + " dy: " + dy);
                System.out.println("ox: " + ox + " oy: " + oy);
                System.out.println("ddx: " + (x - piece.getMoves()[piece.getDir()][1]) + " ddy: "
                        + (y - piece.getMoves()[piece.getDir()][0]));
                if (x < 0 && x > 7 && y < 0 && y > 8)
                    return false;
                if (x != dx && y != dy)
