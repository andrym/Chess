...
public class Game {
    ...
    public void mainLoop() {
        for (int i = 0; i < 10; i++) {
            ...
            resetCoord();
            while ((this.mCoordsPiece[0] < 0 || this.mCoordsPiece[0] > 7) && (this.mCoordsPiece[1] < 0 || this.mCoordsPiece[1] > 7)) {
                System.out.println("Choisissez une piece: ");
                this.mInputPiece = this.mScan.nextLine();
                this.mCoordsPiece = parseCoord(this.mInputPiece);
                if ((this.mCoordsPiece[0] < 0 || this.mCoordsPiece[0] > 7) && (this.mCoordsPiece[1] < 0 || this.mCoordsPiece[1] > 7))
            ...
            while ((this.mCoordsMove[0] < 0 || this.mCoordsMove[0] > 7) && (this.mCoordsMove[1] < 0 || this.mCoordsMove[1] > 7)) {
                System.out.println("Choisissez un move: ");
                this.mInputMove = this.mScan.nextLine();
                this.mCoordsMove = parseCoord(this.mInputMove);
                if ((this.mCoordsMove[0] < 0 || this.mCoordsMove[0] > 7) && (this.mCoordsMove[1] < 0 || this.mCoordsMove[1] > 7))
                    System.out.println("Coordonées de Case invalide (de a à b et de 1 à 8)");
            }
            // System.out.println("Choisissez le move: ");
            // this.mInputMove = this.mScan.nextLine();
            // this.mCoordsMove = parseCoord(this.mInputMove);
            if (this.mBoard.movePiece(this.mCoordsPiece[0], this.mCoordsPiece[1], this.mCoordsMove[0], this.mCoordsMove[1]) == true)
